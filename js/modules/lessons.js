...
// understanding lives here; building & running code lives in the Lab.
import * as S from "../state.js";
import { esc, refresh, toast, confetti, buzz } from "../ui.js";
import { lessonTracks, lessons, lessonById, lessonsInTrack } from "../data/lessons_content.js";
...
// ---- section rendering (content is author-trusted, so HTML is injected as-is) -----------------
...
function renderReader(view, l) {
  ...
  view.innerHTML = `
    ...
    <p class="muted small" style="margin:2px 0 14px">⏱ ${l.minutes} min read · Networking</p>
    <div class="lreader">
...
export default {
  ...
  render(view) {
    if (openId && lessonById(openId)) { renderReader(view, lessonById(openId)); return; }
    ...
    view.innerHTML = `
      ...
      ${lessonTracks.map((t) => `
        ...
        <div class="lcards">${lessonsInTrack(t.id).map(lessonCard).join("")}</div>
      `).join("")}
      <p class="small dim center" style="margin-top:16px">More tracks (low-level, Linux, C, C#…) are being built in this format next.</p>`;
